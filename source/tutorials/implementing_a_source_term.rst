...
In this example, the hydrodynamics is as before the compressible Euler equations with an ideal equation of state. We need to introduce the temperature in the equation of state. We choose to write :math:`p=\rho T`. By setting :math:`T` to a constant defined by the user :math:`T_0`, we can then recompute the pressure using the density of the fluid :math:`p=\rho T_0`.
...
1. Read :math:`\mathbf{U}^*` from the grid 
2. Calculate the kinetic energy :math:`E_k^*`
3. Calculate the pressure :math:`p^{n+1}` using :math:`\rho^*` and :math:`T_0`
4. Calculate the internal energy :math:`e_{int}^n+1`
5. Calculate the total energy :math:`E_{tot}^{n+1}` and write it to the grid
...
Now ``Uin`` and ``Uout`` can be used inside the ``KOKKOS_LAMBDA`` to access the data. Here we ask Dyablo to provide us access to the variables ``rho`` and ``e_tot``. The other variables will not be accessible since we did not ask explicitly for them.

.. note::
 
  **The** ``var_next`` **convention**: In Dyablo we store in memory twice every field. The current value, and the value for the next timestep. To access the value at the current time, use the name of the variable (eg ``rho``). To access the value at the next time, use the name of the variable with ``_next`` (eg ``rho_next``).
...
Add a calculator to your file. In ``Result array name`` type ``p`` and in the field below type the following formula: ``(e_tot - 0.5 * (rho_vx^2 + rho_vy^2)/rho) * 2.0/3.0``. Apply, then create another calculator after the first one. In ``Result array name`` type ``T`` and in the formula ``p/rho``. 
...
.. figure :: ../figs/isothermal_blast.png
  :align: center
