...
The fields available will depend on the plugins activated for the run, but all permanently stored field on the grid are available for outputs. On top of the physical fields, a few annex fields are available : 
...
It is also possible to change solvers (for instance the :doc:`hyperbolic policy <equations>`) to add new physics after a first run using restarts. To do so, one needs to be careful. Changing the solver will treat new physics, but might also work on fields that are nowhere to be found in the restart file. 
